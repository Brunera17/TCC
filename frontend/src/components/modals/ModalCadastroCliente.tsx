...
export const ModalCadastroCliente: React.FC<ModalCadastroClienteProps> = ({
  ...
  const handleSalvar = async () => {
    ...
    setLoading(true);
    try {
      // Dados que serão enviados para o backend (sem abertura_empresa)
      ...
      // Regra de negócio: abertura_empresa é usado apenas no frontend
      const isAberturaEmpresa = formData.cliente.abertura_empresa;
      console.log('🏢 Abertura de Empresa:', isAberturaEmpresa ? 'SIM - Serviço padrão será adicionado' : 'NÃO');
      ...
      if (validation.warnings.length > 0) {
        console.warn('⚠️ Avisos encontrados:', validation.warnings);
      }
      ...
      // Lógica adicional para abertura de empresa
      if (isAberturaEmpresa) {
        console.log('🏢 Cliente criado com flag de abertura de empresa.');
        console.log('📋 TODO: Adicionar serviço padrão de abertura de empresa à proposta.');
        // Aqui você pode:
        // 1. Criar uma proposta automática com serviço de abertura
        // 2. Adicionar um flag especial ao cliente
        // 3. Redirecionar para página de serviços
        showSuccess('Cliente cadastrado com sucesso!', 'Serviço de abertura de empresa será adicionado automaticamente.');
      } else {
